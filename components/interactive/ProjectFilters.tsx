...
export function ProjectFilters({ projectCounts }: ProjectFiltersProps) {
  const [activeFilter, setActiveFilter] = useState<ProjectFilter>("all");

  // This data attribute keeps the grid server-rendered while CSS handles visibility.
  useEffect(() => {
    ...
    <div className="flex items-center justify-between flex-wrap gap-4 border-b border-zinc-900 pb-4">
      <div className="flex items-center gap-2 font-mono text-xs text-zinc-500">
        ...
        <span className="text-zinc-300 font-bold">
          {projectCounts[activeFilter]} reposit\u00f3rio(s)
        </span>
      </div>
