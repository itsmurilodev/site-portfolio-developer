      ...
      <div className="relative w-full max-w-md rounded-md border border-white/[0.08] bg-bg-deep/85 p-6 backdrop-blur-md shadow-2xl">
          ...
          {logs.map((log, index) => {
            ...
            return (
              <div key={index} className="flex gap-1.5 items-start">
                ...
                <span className={cn(log.includes("[ok]") && "text-emerald-400", log.includes("[online]") && "text-terminal-orange-soft")}>
                  {log}
                </span>
              </div>
            );
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex justify-between font-mono text-[10px] text-zinc-500">
            ...
            <span className="text-terminal-orange-soft font-semibold">{Math.min(progress, 100)}%</span>
          </div>
